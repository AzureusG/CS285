...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                next_qs += self.temperature * next_action_entropy.expand(
                    (self.num_critic_networks, batch_size)
                )

            # Compute the target Q-value
            flag = (1 - done.to(torch.int8)).expand(self.num_critic_networks, batch_size)
            target_values: torch.Tensor = reward.expand(self.num_critic_networks, batch_size) + flag * self.discount * next_qs
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        sample1 = 1000
        sample_actions = action_distribution.rsample((sample1,)) #rsample允许梯度回传，适用于SAC/PPO等策略梯度方法
        logprobs = action_distribution.log_prob(sample_actions)
        entropy = -torch.mean(logprobs, dim=0)
        return entropy
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            action = action_distribution.rsample((self.num_actor_samples,))  # Use rsample for gradients
            assert action.shape == (
