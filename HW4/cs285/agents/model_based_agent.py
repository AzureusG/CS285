...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        normal_obs_acs = (obs_acs - self.obs_acs_mean) / (self.obs_acs_std + 1e-8)
        pred_delta = self.dynamics_models[i](normal_obs_acs)
        target_delta = (next_obs - obs - self.obs_delta_mean) / (self.obs_delta_std + 1e-8)
        loss = self.loss_fn(pred_delta, target_delta)
    ...
    def update_statistics(self, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        self.obs_delta_mean = torch.mean(next_obs - obs, dim=0, keepdim=True)
        self.obs_delta_std = torch.std(next_obs - obs, dim=0, keepdim=True)
    ...
    def evaluate_action_sequences(self, obs: np.ndarray, action_sequences: np.ndarray):
        ...
        for acs in action_sequences.transpose(1, 0, 2):
            ...
            rewards = rewards.reshape(self.ensemble_size, -1)  # shape (ensemble_size, mpc_num_action_sequences, ac_dim)
    ...
    def get_action(self, obs: np.ndarray):
        ...
        if self.mpc_strategy == "random":
            ...
            for i in range(self.cem_num_iters):
                ...
                elite_sequences = action_sequences[tops]
                if i == 0:
